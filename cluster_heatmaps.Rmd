...
cat("Datasets: threshold at:", TXT_THRESHOLD,"\n")
cat("include_x: reps not merged:", dim(include_x),"\n")
cat("include3d_x: reps merged:", dim(include3d_x),"\n")
```
...
pobj$kmeans = reorder_kmeans(pobj$kmeans)
kclusters = pobj$kmeans$cluster
set.seed(31415)
kmeans_on3d_k4 = reorder_kmeans(kmeans(include3d_x,nclust))
set.seed(31415)
kmeans_on3d_k4 =kmeans(include3d_x,nclust)
#### work with the 3d matrix as a circle in 2D
source('scripts/rotation_matrices.R')
data2d = flatten_std( as.matrix( cbind(include3d_x,1)) ) # this seems not to be right, but since it's for vis. purposes, 
...
### merge the centers that are within-stage replicates
