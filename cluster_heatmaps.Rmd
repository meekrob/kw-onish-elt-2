...
data3d = data.frame( LE=rowMeans(data[,c(1,2)]),
                     L1=rowMeans(data[,c(3,4)]),
                     L3=rowMeans(data[,c(5,6)]))
...
### compare cluster assignments of 6D data versus 3D data
# this table will allow you to translate the assigned cluster number
# to a rank that reflects the number of data points in each cluster.
# The rank is comparable across different runs, different data varieties 
# of this dataset, as long as the compared datasets are of the same row number:
# e.g.: kclusters based on LE,L1,L3 matrix versus [LE,L1,L3]*[replicates]
# or different seeds of the same run
build_trans_table <-function(knumbers) { 
  count = table(knumbers); 
  count = sort(count); 
  k = as.numeric(names(count)); 
  rank=length(k):1; 
  trans=cbind(k,rank,count); 
  trans = trans[order(k),]; 
  return(trans);
}
translator <- function(x, translate) 
{ 
  td = translate[x,2]; names(td) <- NULL; return(td);
}

k4data3d = kmeans(data3d,4)
k_3d_k4 = k4data3d$cluster
k_6d_k4 = kclusters

t3d = build_trans_table(k_3d_k4)
t6d = build_trans_table(k_6d_k4)
cor( translator(k_3d_k4,t3d), translator(k_6d_k4,t6d))
stopifnot(cor( translator(k_3d_k4,t3d), translator(k_6d_k4,t6d)) > .99) 
########## correlation between cluster assigments must be at least this high or the script will stop ####

#### work with the 3d matrix as a circle in 2D
source('~/work/onish_ChIP_R_Analysis/rotation_matrices.R')
data2d = flatten_std( as.matrix( cbind(peaks3d_std,1)) ) # this seems not to be right, but since it's for vis. purposes, 
# xy coordinates range between -1,1, and polar coordinates all have radius=1, so I'm sticking with it for now.
data2d = data2d[,1:3]
k11data3d = kmeans(data3d,11)
