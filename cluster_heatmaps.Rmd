...
dfG_metadata %>% as.data.frame() %>% select(-starts_with('min'), -starts_with('mean'), -starts_with('N')) %>%
  rename(
...
# empirical data
feature_table = readRDS("feature_mapping_empirical.tbl") %>% as_tibble() %>% rename('unmapped ± 5Kb'='unmapped - 5Kb') %>% as.matrix()
...
features_observed %>% melt() %>% rename(feature=Var1,count=value) -> feat_obs_tbl
...
melt(
  ...
  variable.name = "feature",
  value.name = "count"
) %>% as_tibble() %>% rename(feature = Var1) %>% mutate(feature = factor(feature, levels = fiveToThree))
