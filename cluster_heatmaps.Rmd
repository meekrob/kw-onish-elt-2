...
clusters$k11_trends = clusters$kmeans_on3d_k11$centers
rownames(clusters$k11_trends) <- as.character(clusters$kmeans_on3d_k11$size)

clusters$k11_trends_long = melt(clusters$k11_trends)
colnames(clusters$k11_trends_long) <- c("cluster size", "stage", "center")
clusters$k11_trends_long$`cluster size` = clusters$k11_trends_long$`cluster size`
clusters$k11_trends_long$`size < 700` = clusters$k11_trends_long$`cluster size` 
clusters$k11_trends_long$`size < 700`[clusters$k11_trends_long$`cluster size` < 700] = 'cluster size < 700'
clusters$k11_trends_long$`size < 700`[clusters$k11_trends_long$`cluster size` >700] = 'cluster size > 700'
clusters$k11_trends_long$`cluster size` = as.factor(clusters$k11_trends_long$`cluster size`)
label=paste(sprintf("%d cluster trends at std. dev. > %.3f,", nrow(clusters$k11_trends), reps_avd$threshold3d_q), "(excludes lower ", sprintf("%.1f%%)", THRESHOLD*100))
figure_colors = scale_colour_manual(values=brewer.pal(11,"Paired"))
print(
  ggplot(clusters$k11_trends_long, 
         aes(x=stage,y=center, 
             group=`cluster size`,
             colour=`cluster size`)) + 
    geom_line(size = 1.5) + 
    labs(title="Kmeans cluster centers (k=11)", 
         y = "cluster center (Z)", 
         x = "developmental stage") + 
    figure_colors + facet_grid(~ `size < 700`))
...
uint  k11cluster; \"Cluster assignment in kmeans = 4, 0 for not-changing\"
...
bedToBigBed peaksForBigBed.bed chrom.sizes peaksForBigBed.bb -type=bed3+15 -as=peaks.as
```
...
# normalized data
all_nan_rows = apply(df_max, 1, function(x) { all(!is.finite(x))}); # this vector is now all FALSE due to upstream filtering
stopifnot(all(! all_nan_rows)) # but check here
...
norm_LE_1 = row_scaled_x[,1]
norm_LE_2 = row_scaled_x[,2]
norm_L1_1 = row_scaled_x[,3]
norm_L1_2 = row_scaled_x[,4]
norm_L3_1 = row_scaled_x[,5]
norm_L3_2 = row_scaled_x[,6]
