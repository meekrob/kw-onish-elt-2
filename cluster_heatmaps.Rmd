...
source("check_for_install_deps.R",echo = T) 
...
library(grid)
library(VennDiagram)
library(plyranges) # tidy the GRange datatypes
...
df %>% dplyr::select(starts_with('max')) %>%
  dplyr::rename(
    max_log_LE_1_minus_log_LE_input=max_log_LE_1_minus_log_LE_input_z.bw,  
         max_log_LE_2_minus_log_LE_input=max_log_LE_2_minus_log_LE_input_z.bw,
         max_log_L1_1_minus_log_L1_input=max_log_L1_1_minus_log_L1_input_z.bw, 
         max_log_L1_2_minus_log_L1_input=max_log_L1_2_minus_log_L1_input_z.bw, 
         max_log_L3_1_minus_log_L3_input=max_log_L3_1_minus_log_L3_input_z.bw, 
         max_log_L3_2_minus_log_L3_input=max_log_L3_2_minus_log_L3_input_z.bw 
...
dfG_metadata %>% as.data.frame() %>% dplyr::select(-starts_with('min'), -starts_with('mean'), -starts_with('N')) %>%
  dplyr::rename(
         L1_1=max_log_L1_1_minus_log_L1_input_z.bw, 
         L1_2=max_log_L1_2_minus_log_L1_input_z.bw, 
         L3_1=max_log_L3_1_minus_log_L3_input_z.bw, 
         L3_2=max_log_L3_2_minus_log_L3_input_z.bw, 
         LE_1=max_log_LE_1_minus_log_LE_input_z.bw,  
         LE_2=max_log_LE_2_minus_log_LE_input_z.bw) %>%
  dplyr::select( # to reorder them
    LE_1,
    LE_2,
    L1_1,
...
grid.newpage()
grid.draw(draw.triple.venn(area1, area2, area3, n12, n23, n13, n123, scale=F,euler.d=F,fill=c('red','green','blue')))
...
ra = read.table('right.answers')
cat("The following don't match expected mappings:")
ra[stacked_nr[ ra[[1]] ]$feature != ra[[3]],]

## Spread sheet for spot-checks
xl = data.frame(peak.position = sprintf("%s:%d-%d", seqnames(stacked_nr),start(stacked_nr),end(stacked_nr)))
xl$peak = stacked_nr$name
xl$old.mapping = sprintf("%s (%s)", ap$gene_name, ap$feature)
xl$new.mapping = sprintf("%s (%s)", stacked_nr$gene_name, stacked_nr$feature)
xl$old.insideFeature = ap$insideFeature
xl$new.insideFeature = stacked_nr$insideFeature
xl$old.distance = ap$shortestDistance
xl$new.distance = stacked_nr$shortestDistance
xl = xl %>% mutate(mapping.changed = old.mapping != new.mapping)
# randomization column
set.seed(13)
xl$randoms=sample(1:nrow(xl),nrow(xl))
xl = xl %>% arrange(! mapping.changed, randoms)
write.table(xl, "featureMapping.txt",quote = F, row.names=F, sep="\t")
...
# 
# ap.tbl %>% mutate(spenc_LE=feature %in% rownames(LEL2)[LEL2[,'spenc_LE']==1],
#                   spenc_L2=feature %in% rownames(LEL2)[LEL2[,'spenc_L2']==1]) -> ap.tbl
# 
# # apply IDR rules
...
# 
# # sort each class, attach rank, and merge
# # sorting least sig. to most sig. to affect how the points
# # are plotted on top of each other below. 
# # To select regions, re-sort, within k4cluster, on rankInCluster
# ap.tbl.0 %>% arrange((selectionScore)) %>% 
#   mutate(rankInCluster = nrow(ap.tbl.0):1) -> ap.tbl.scored
# ap.tbl.1 %>% arrange((selectionScore)) %>%
#   mutate(rankInCluster = nrow(ap.tbl.1):1) %>%
#   bind_rows(ap.tbl.scored) -> ap.tbl.scored
# ap.tbl.2 %>% arrange((selectionScore)) %>% 
#   mutate(rankInCluster = nrow(ap.tbl.2):1) %>%
#   bind_rows(ap.tbl.scored) -> ap.tbl.scored
# ap.tbl.3 %>% arrange((selectionScore)) %>% 
#   mutate(rankInCluster = nrow(ap.tbl.3):1) %>%
#   bind_rows(ap.tbl.scored) -> ap.tbl.scored
# ap.tbl.4 %>% arrange((selectionScore)) %>% 
#   mutate(rankInCluster = nrow(ap.tbl.4):1) %>%
#   bind_rows(ap.tbl.scored) -> ap.tbl.scored
# 
# # write an Excel spreadsheet
# ##
# xlsFilename="topScorers.xlsx"
# topScorersXLS = loadWorkbook(xlsFilename, create=T)
# 
# sheetname = format(Sys.time(), "K1 %y-%m-%d-%H-%M")
# createSheet(topScorersXLS, sheetname)
# ap.tbl.1 %>% arrange(desc(selectionScore)) %>%
#   mutate(rankInCluster = 1:nrow(ap.tbl.1)) %>% 
#   head(20) -> wrkSheet
#   writeWorksheet(topScorersXLS, sht, sheetname)
# 
# # ap.tbl.1 %>% arrange(desc(selectionScore)) %>%
# #     mutate(rankInCluster = 1:nrow(ap.tbl.1)) %>% 
# #     filter(spenc_LE | spenc_L2) %>% head(5) %>%
# #     write.table(file ="k1_top5spencer.txt",quote=F,sep="\t",row.names=F)
# 
# sheetname = format(Sys.time(), "K2 %y-%m-%d-%H-%M")
# createSheet(topScorersXLS, sheetname)
# ap.tbl.2 %>% arrange(desc(selectionScore)) %>%
#   mutate(rankInCluster = 1:nrow(ap.tbl.2)) %>% 
#   head(20) ->wrkSht 
#   writeWorksheet(topScorersXLS, wrkSht, sheetname)
# 
# # ap.tbl.2 %>% arrange(desc(selectionScore)) %>%
# #     mutate(rankInCluster = 1:nrow(ap.tbl.2)) %>% 
# #     filter(spenc_LE | spenc_L2) %>% head(5) %>%
# #     write.table(file ="k2_top5spencer.txt",quote=F,sep="\t",row.names=F)
# 
# sheetname = format(Sys.time(), "K3 %y-%m-%d-%H-%M")
# createSheet(topScorersXLS, sheetname)
# ap.tbl.3 %>% arrange(desc(selectionScore)) %>%
#   mutate(rankInCluster = 1:nrow(ap.tbl.3)) %>% 
#   head(20) ->wrkSht
#   writeWorksheet(topScorersXLS, sht, sheetname)
#   
# # ap.tbl.3 %>% arrange(desc(selectionScore)) %>%
# #     mutate(rankInCluster = 1:nrow(ap.tbl.3)) %>% 
# #     filter(spenc_LE | spenc_L2) %>% head(5) %>% 
# #     write.table(file ="k3_top5spencer.txt",quote=F,sep="\t",row.names=F)
# 
# sheetname = format(Sys.time(), "K4 %y-%m-%d-%H-%M")
# createSheet(topScorersXLS, sheetname)  
# ap.tbl.4 %>% arrange(desc(selectionScore)) %>%
#   mutate(rankInCluster = 1:nrow(ap.tbl.4)) %>% 
#   head(20) -> wrkSht
#   writeWorksheet(topScorersXLS, sht, sheetname)
#   
# saveWorkbook(topScorersXLS)
# 
# # for the plot
# factorRanks = function(x) {
#   strats=ifelse(x <= 10, "top 10", ifelse(x<=20, "top 20", ifelse(x<=50, "top 50", "remainder")))
#   return(factor(strats, levels=c("top 10","top 20","top 50","remainder")))
# }
# 
# ap.tbl.scored %>% mutate(rankLevels = factorRanks(ap.tbl.scored$rankInCluster)) -> ap.tbl.scored
# 
# ggplot(
#   ap.tbl.scored %>% filter(k4cluster > 0),
#   aes(
#     x = variance,
#     y = k4weights,
#     alpha=rankLevels,
#     color=rankLevels
#   )
# ) + geom_point() + facet_wrap( ~ k4cluster) + scale_color_manual(values=c("top 10" = "dark blue","top 20"="blue", "top 50" = "light blue", "remainder" = "grey")) +
#   scale_alpha_manual(values=c("top 10" = 1,"top 20" = 1, "top 50" = 1, "remainder" = .2)) +
#   ggtitle("Peaks most representative of each developmental trend", subtitle = "Ranked by variance*cluster score [0-1]") 
# 
# ggplot(
#   ap.tbl.scored %>% filter(k4cluster == 0),
#   aes(y = sqrt(variance), x = coeffVariation, color = rankInCluster <= 20)
# ) + geom_point() + scale_color_manual(values=c('grey','blue'))
# 
...
# You must connect to the NAS first to get these genes.
if (!('LEL2' %in% ls())) {source('SpencerGenes.R')}
upset(matchtable(genes_w_ELT2_EmbryoHigh_peak=ap.tbl.1$feature,matchObj = LEL2))
upset(matchtable(genes_w_ELT2_LarvaHigh_peak=ap.tbl.2$feature,matchObj = LEL2))
upset(matchtable(genes_w_ELT2_L3High_peak=ap.tbl.3$feature,matchObj = LEL2))
upset(matchtable(genes_w_ELT2_DevIncreasing_peak=ap.tbl.4$feature,matchObj = LEL2))
```
...
GOSummary<- function(GOdata, topNodes = 50) {
  ...
  return(GenTable(object=GOdata, weight=l$weight, classic=l$classic, elim=l$elim, KS=l$KS, orderBy="weight",ranksOf = "classic", topNodes = topNodes))
}
