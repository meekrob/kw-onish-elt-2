...
for (THRESHOLD in THRESHOLDS) {
  ...
  nclust=4
  set.seed(31415)
  pobj = pheatmap(threshold_x, kmeans=nclust, cluster_rows=F, cluster_cols = F, main=paste("excluding lower",TXT_THRESHOLD, "sd", sep=" "))
  ...
  for (k_i in names(split_non_normalized)) { 
    ...
    kcluster_filename = paste("allStagesUNION_max_sd_", TXT_THRESHOLD, "_", k_i, ".bedlike", sep="")
  ...
  pheatmap(threshold_x[korder,], cluster_rows=F, cluster_cols=F, show_rownames=F, main=paste("excluding lower",THRESHOLD, "sd", sep=" "))
  ...
  label=paste("threshold=",TXT_THRESHOLD)
  print(ggplot(trends_ordered_long, aes(x=stage,y=center, group=description,colour=description )) + geom_line() + labs(title=label, y = "cluster center"))
  
  
  if (FALSE) {
    plot(c(1,3), c(min(patterns),2), type='n', axes=F, main=paste("Cluster centers, excluding lower",TXT_THRESHOLD, "sd", sep=" "),xlab = "Stage", ylab="Normalized value")
