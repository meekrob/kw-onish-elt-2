---
title: "201208_IM_JM149_Rep1_Analysis"
author: "IRM"
date: "201208"
output:
...
** using excel file with only L1 L3 and adult data
...
```{r}
sheets <- c("Rep1_ELT-2_RNAi",  "Rep1_L4440_RNAi")


image_df <- data.frame()
for(sheet in sheets) {
 toappend <- read_excel("./JM149_Rep1_elt-2_Promoter_ImageJ_Analysis_FilteredStages.xlsx", sheet = sheet)
...
Save Plot
```{r}
ggsave("201119_JM149_ELT-2_RNAi_Development_Series_Abbreviated.pdf", width = 7.5, height = 5)
```
...
save plot
```{r}
ggsave("201125_JM149_ELT-2_RNAi_Development_Series_Abbreviated_Area_Included.pdf", width = 7.5, height = 5.5)
```
...
save plot
```{r}
ggsave("201119_JM149_ELT-2_RNAi_Development_Series_Abbreviated_L4440_Area_Included.pdf", width = 7.5, height = 4.5)
```
...
```{r}
ggsave("201125_JM149_Relative_Promoter_Fluorescence_Abbreviated.pdf", width = 6, height = 5)
```
