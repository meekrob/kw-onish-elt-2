...
```
Perform phenotype analysis
```{python}
tea_dict = phenotype
classA_phenotype = tea.enrichment_analysis(classA["WBGeneID"],tea_dict[tea_dict.wbid.isin(all_bound.WBGeneID)])
classB_phenotype = tea.enrichment_analysis(classB["WBGeneID"], tea_dict[tea_dict.wbid.isin(all_bound.WBGeneID)])
classC_phenotype = tea.enrichment_analysis(classC["WBGeneID"], tea_dict[tea_dict.wbid.isin(all_bound.WBGeneID)])
classD_phenotype = tea.enrichment_analysis(classD["WBGeneID"], tea_dict[tea_dict.wbid.isin(all_bound.WBGeneID)])

```
```{r}
tea_phenotype_df <- bind_rows(data.frame(py$classA_phenotype, class = "classA"),
                           # data.frame(py$classB_phenotype, class = "classB"),
                           data.frame(py$classC_phenotype, class = "classC"),
                           data.frame(py$classD_phenotype, class = "classD")
)

tea_phenotype_plot <- tea_phenotype_df %>% 
  filter(Q.value < 0.05,  Observed > 5) %>% 
  ggplot(aes(x = reorder_within(Term, -log10(Q.value), class), y = -log10(Q.value))) +
  geom_point(aes(size = Observed)) +
  coord_flip() +
...
```{python}
tea_dict = go
classA_go = tea.enrichment_analysis(classA["WBGeneID"],tea_dict[tea_dict.wbid.isin(all_bound.WBGeneID)])
classB_go = tea.enrichment_analysis(classB["WBGeneID"], tea_dict[tea_dict.wbid.isin(all_bound.WBGeneID)])
classC_go = tea.enrichment_analysis(classC["WBGeneID"], tea_dict[tea_dict.wbid.isin(all_bound.WBGeneID)])
classD_go = tea.enrichment_analysis(classD["WBGeneID"], tea_dict[tea_dict.wbid.isin(all_bound.WBGeneID)])

```
```{r}
tea_go_df <- bind_rows(data.frame(py$classA_go, class = "classA"),
                           data.frame(py$classB_go, class = "classB"),
                           data.frame(py$classC_go, class = "classC"),
                           data.frame(py$classD_go, class = "classD")
)

tea_go_plot <- tea_go_df %>% 
  filter(Q.value < 0.05,  Observed > 5) %>% 
  ggplot(aes(x = reorder_within(Term, -log10(Q.value), class), y = -log10(Q.value))) +
  geom_point(aes(size = Observed)) +
  coord_flip() +
