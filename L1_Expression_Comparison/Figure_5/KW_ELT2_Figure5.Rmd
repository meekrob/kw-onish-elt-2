...
bound_classes <- paste("Target Gene Class", c("A1", "A2", "B", "C"), sep = " ")
...
write_csv(bound_tissue_genes, file = "./02_output/FileS5_ELT2-Targetgeneclass-Ontology_results.csv")
```
...
# write.csv(bound_phenotype_genes, file = "./02_output/Figure5_ELT2-Bound-DE_Phenotype-Ontology_results.csv")
```
...
head(bound_go_genes)
# write.csv(bound_go_genes, file = "./02_output/Figure5_ELT2-Bound-DE_Gene-Ontology_results.csv")
```

# Figure S6 Enrichment of ELT-2 binding in each Dineen SET

Use hypergeometric to determine if there is more ELT-2 binding in each set than expected by chance compared within all six SETS
...
```
Use hypergeometric to determine if there is more ELT-2 binding in each Dineen SET than expected by chance compared to the whole genome
...
```
Output for supplemental files with genes

```{r}
elt2_tea_genes <- bind_rows(tissue_df, phenotype_df, go_df) %>% 
  filter(wbid %in% (elt2_peaks %>% filter(cluster.description %in% elt2_cluster_names))$WBGeneID) %>% pivot_longer(-wbid) %>% filter(value == 1, name %in% elt2_tea_df$Term) %>% 
  right_join(elt2_peaks[c("WBGeneID")], by = c("wbid"= "WBGeneID")) %>% 
  dplyr::select(-value) %>%
  inner_join(elt2_tea_df , by = c("name" = "Term")) %>%
  filter(cluster %in% elt2_cluster_names) %>%
  arrange(analysis)
head(elt2_tea_genes)
# write_csv(elt2_tea_genes, file = "02_output/FileS6_ELT2_occupancy_ontology_results.csv")
```
...
colnames(dineen_dev_pearson_residuals) <-
  c("Dineen_Class", "Intestine_cluster", "Pearson_Residual", "Observed")
# set factor levels
...
# set factor levels
dineen_dev_pearson_residuals$Intestine_cluster <-
  factor(
    dineen_dev_pearson_residuals$Intestine_cluster,
    levels = c(
      "Embryonic",
...
dineen_dev_contingency <- dineen_dev_pearson_residuals %>% dplyr::select(Expression.cluster = Dineen_Class, Intestine.cluster = Intestine_cluster, Genes.observed = Observed, Pearson.residual = Pearson_Residual, p.value, p.adjust)

# write_csv(dineen_dev_contingency , file = "./02_output/TableS6_chi-square_targetgeneclass_intestinecluster.csv")
```
