...
elt2_gene_occupancy %>% group_by(single_cluster) %>% summarise(gene_total = n()) %>% ggplot(aes(x = single_cluster, y = gene_total)) +
...
# Figure S4: How many intestine genes are bound by ELT-2 at any stage regardless of occupancy cluster?

```{r}
intestine_gene_list
intestine_peaks <- elt2_peaks %>% group_by(WBGeneID) %>% summarise(peaks = n()) %>% right_join(intestine_gene_list, by = "WBGeneID") %>% select(WBGeneID, peaks) %>% replace_na(list(peaks = 0)) %>% mutate(status = case_when(peaks == 0 ~ "not_bound", peaks >= 1 ~ "bound"))
...
bound_intestine_genes <-  intestine_hyper %>% filter(status == "bound") %>% select(genes) %>% as.numeric() # the number of successes in the sample size, the number of bound genes within one set
...
whole_and_intestine_binding <- data.frame(
  ...
  genes = c(whole_genome_bound, whole_genome_notbound, bound_intestine_genes, intestine_hyper %>% filter(status == "not_bound") %>% select(genes) %>% as.numeric()),
...
# Figure S4: Scree plot analysis of developmental intestine gene expression
