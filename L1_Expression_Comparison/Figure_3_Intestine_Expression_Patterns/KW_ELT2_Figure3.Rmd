...
_No longer using this dataset_
...
intestine_upset_plot <- UpSet(m1, 
      set_order = c("hash_emb", "spen_LE", "spen_L2", "pauli_L4"),
...
chip_rna_matrix <-
  chip_rna_df %>% filter(WBGeneID %in% single_cluster_genes$WBGeneID) %>% dplyr::select(emb_510min:YA) %>% as.matrix()
...
chip_rna_df_nr_top95 <- chip_rna_df_nr[real&changing,]
...
rna_cluster_names <- c("Larval", "Embryonic", "Adult")
# 4. Draw the heatmap
...
```{r}
chip_rna_df_wider <- chip_rna_df %>% group_by(WBGeneID, cluster.description) %>% summarise(cluster.count = n()) %>% pivot_wider(id_cols = WBGeneID, names_from = cluster.description, values_from = cluster.count, values_fill = 0) %>% ungroup()
chip_rna_df_wider["Not_Bound" > 1 & "Increasing" == 0]
chip_rna_df_wider$Not_Bound > 1 & chip_rna_df_wider$Increasing == 0

chip_rna_df_wider %>% filter(Not_Bound >= 1 & Increasing == 0, L3_High == 0, Larval == 0, Not_Changing == 0, Embryo_Specific == 0)

chip_rna_df_wider %>% filter(Not_Bound == 0 & Increasing >= 1 , L3_High == 0, Larval == 0, Not_Changing == 0, Embryo_Specific == 0)

chip_rna_df_wider %>% filter(Not_Bound == 0 & Increasing == 0, L3_High >= 1, Larval == 0, Not_Changing == 0, Embryo_Specific == 0)

chip_rna_df_wider %>% filter(Not_Bound == 0 & Increasing == 0, L3_High == 0, Larval >= 1, Not_Changing == 0, Embryo_Specific == 0)

chip_rna_df_wider %>% filter(Not_Bound == 0 & Increasing == 0, L3_High == 0, Larval == 0, Not_Changing >= 1, Embryo_Specific == 0)

chip_rna_df_wider %>% filter(Not_Bound == 0 & Increasing == 0, L3_High == 0, Larval == 0, Not_Changing == 0, Embryo_Specific >= 1)

```

```{r}
single_cluster_genes <- chip_rna_df_wider %>%
  mutate(
    single_cluster =
      case_when(
        Not_Bound >= 1 &
