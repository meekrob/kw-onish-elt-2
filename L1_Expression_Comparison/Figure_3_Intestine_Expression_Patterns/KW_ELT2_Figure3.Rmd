...
# How many intestine genes are bound by ELT-2 at any stage regardless of occupancy cluster?
...
exp.x <- hyper_expected(x = bound_intestine_genes, k = all_intestine_genes, m = all_elt2_bound_genes, n = all_elt2_notbound_genes)
exp.x
percent_of_intestine_bound <- intestine_hyper %>% 
  mutate(type = "Intestine_Genes", gene_percent = 100*genes/all_intestine_genes) %>% 
...
)) %>% filter(status != "not_bound") %>% ggplot(aes(x = status, y = genes)) + geom_bar(stat = "identity") + theme_classic()
obs_expect_intestine_bound
if (plot == TRUE){
ggsave(paste(plotdir,
          "FigureS4_Percent_Intestine_Genes_ELT2_Bound",
          "_",
          today(),
          ".pdf",
          sep = ""), 
       plot = percent_of_intestine_bound,
       width = 3, height = 3, 
       useDingbats=FALSE)
ggsave(paste(plotdir,
