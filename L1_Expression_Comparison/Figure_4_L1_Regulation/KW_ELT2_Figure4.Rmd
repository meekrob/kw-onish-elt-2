...
if (do_plot == TRUE){
myPDFplot(Ha_L1chip, "FigureS5_DE_Heatmap_elt2elt7DERNAseq_L1elt2bound", 4, 4.5, plotdir)
}
```
...
myggsave(
  ...
  name = "FigureS5_percentage_l1bound_per_expression_cluster",
  width = 4,
  height = 5,
  plotdir = plotdir
)
...
# bound_only_class$class <- factor(bound_only_class$class, levels = c("ClassA", "ClassB", "ClassC", "ClassD"))
# write_csv(bound_only_class %>% mutate(class = case_when(class == "ClassA" ~ "Target Gene Class A1", 
#                                               class == "ClassB" ~ "Target Gene Class A2", 
#                                               class == "ClassC" ~ "Target Gene Class B", 
#                                               class == "ClassD" ~ "Target Gene Class C")),
#           file = "./02_output/ELT-2_Target_Gene_Class_Assignments.csv")
# Read in Class assignments
bound_only_class <- read_csv(file = "./02_output/ELT-2_Target_Gene_Class_Assignments.csv")
bound_only_class$class <- factor(bound_only_class$class, levels = c("Target Gene Class A1", "Target Gene Class A2", "Target Gene Class B", "Target Gene Class C"))
...
```
# Make outout of bound gene expression values

```{r}
dynamic_counts_matrix_unscaled_bound_only <- matrix_select(dynamic_counts_matrix, l1_bound_list$WBGeneID)
write_csv(as.data.frame(dynamic_counts_matrix_unscaled_bound_only) %>% rownames_to_column(var = "WBGeneID"), 
          file = "./02_output/Figure4_Dynamic_Counts_BoundOnly.csv")
```

Further subset this heatmap for transcription factors

```{r}
intestine_gene_list <- read.csv("../Figure_3_Intestine_Expression_Patterns/02_output/Fig3_Public_Intestine_RNA_Gene_List.csv", stringsAsFactors = FALSE)
wTF3.0 <-
...
HAboundTF_class <- Heatmap(
  ...
  bottom_annotation = HeatmapAnnotation(
    ...
    col = list(Class = c(
      "Target Gene Class A1" = "red", "Target Gene Class A2" = "orange", "Target Gene Class B" = "yellow", "Target Gene Class C" = "green"
    )),
    border = TRUE
  ) + rowAnnotation(Intestine = rownames(dynamic_counts_matrix_scaled_TFs_bound_names) %in% intestine_gene_list$Public_name, col = list(Intestine = c( "TRUE"= "black","FALSE"= "white")), border = TRUE)
...
if (do_plot == TRUE){
  myPDFplot(
    ...
    name = "FigureS7_Bound-DE_TFs_heatmap",
    height = 5,
    width = 5.5,
    plotdir = plotdir
  )
...
```{r}
tableS6 <- bound_tf_class %>% left_join(read.csv("./01_input/TF3-0_namesonly.txt",
           sep = "\t",
           header = TRUE) %>% dplyr::select(-(X:X.3)), by = "WBGeneID") %>% dplyr::select(WBGeneID, Target.class = class, Sequence.name, Public.name = Public_name, DBD) %>% bind_cols(Intestine.specific = bound_tf_class$WBGeneID %in% intestine_gene_list$WBGeneID)
table(tableS6$Intestine.specific)
table(tableS6$Intestine.specific)/length(tableS6$Intestine.specific)
# write_csv(tableS6, file = "./02_output/TableS7_targetgeneclass_TFs.csv")
```
...
# Identify single binding type genes for bound class genes
...
dineen_ELT2_pearson_residuals$Dineen_Class <-
  factor(
    dineen_ELT2_pearson_residuals$Dineen_Class,
    levels = c("Target Gene Class C", "Target Gene Class B", "Target Gene Class A2", "Target Gene Class A1")
  )
# set factor levels
...
```{r}
dineen_contingency <- dineen_ELT2_pearson_residuals %>% dplyr::select(Expression.cluster = Dineen_Class, Occupancy.cluster = ELT2_cluster, Genes.observed = Observed, Pearson.residual = Pearson_Residual, p.value, p.adjust)

head(dineen_contingency)
# write_csv(dineen_contingency, file = "./02_output/TableS5_chi-square_targetgeneclass_vs_occupancy.csv")
```
...
# Pull DNA sequences for MEME motif analysis

```{r}
library(Biostrings)
library(BSgenome.Celegans.UCSC.ce11)
as.data.frame(elt2_GRange)
elt2_peaks

all_elt2_summit <-
  as.data.frame(elt2_GRange) %>% dplyr::rename(WBGeneID = "feature") %>% full_join(bound_only_class, by = "WBGeneID") %>% dplyr::select(seqnames:end, name, WBGeneID, k4labels, class, LE_peak_1_summit:L3_peak_2_summit) %>% rowwise() %>% mutate(peak_summit = round(mean(
    c(LE_peak_1_summit, LE_peak_2_summit, 
      L1_peak_1_summit, L1_peak_2_summit, 
      L3_peak_1_summit, L3_peak_2_summit), na.rm = TRUE
  ))) %>%
  rowwise() %>%
  mutate(
    summit_start = peak_summit - 250,
    summit_end = peak_summit + 250,
    peak_width = summit_end - summit_start
  ) %>% filter(!is.na(seqnames))

all_elt2_summit$class <- factor(all_elt2_summit$class, levels = c("Target Gene Class A1", "Target Gene Class A2", "Target Gene Class B", "Target Gene Class C", "no_class"))
all_elt2_summit$class[is.na(all_elt2_summit$class)] <- "no_class"
head(all_elt2_summit)
```
...
elt2_summit_GRange <- trim(makeGRangesFromDataFrame(all_elt2_summit,
...
elt2_summit_GRange$bound_class <- all_elt2_summit$class
elt2_summit_GRange$feature <- all_elt2_summit$WBGeneID
elt2_summit_GRange$name <- all_elt2_summit$name
elt2_summit_GRange$k4labels <- all_elt2_summit$k4labels
names(elt2_summit_GRange) <- all_elt2_summit$name
head(elt2_summit_GRange)
length(elt2_summit_GRange)
```

Get DNA sequences for all ELT-2 binding sites

```{r}
head(getSeq(Celegans, elt2_summit_GRange))
# export(getSeq(Celegans, elt2_summit_GRange), "./02_output/MEME-ChIP_fasta/all_elt2_summits.fasta")
```
...
```{r}
# export by class
classnames <- paste(rep("Class", 4), LETTERS[1:4], sep = "")
for(i in classnames){
  filename <- paste("./02_output/MEME-ChIP_fasta/", i, "_peaks.fasta", sep = "")
  export(getSeq(Celegans, elt2_summit_GRange[elementMetadata(elt2_summit_GRange)[,"bound_class"] == i]), filename)
}
# export all class A through D
export(getSeq(Celegans, elt2_summit_GRange[elementMetadata(elt2_summit_GRange)[,"bound_class"] %in% classnames]), "./02_output/MEME-ChIP_fasta/TargetGeneClassA1throughC_elt2_summits.fasta")
# export all peaks with one class removed
export(getSeq(Celegans, elt2_summit_GRange[elementMetadata(elt2_summit_GRange)[,"bound_class"] %in% 
                                      c("Target Gene Class A2","Target Gene Class B", "Target Gene Class C")]),
       "./02_output/MEME-ChIP_fasta/BoundDE_NoClassA1_elt2_summits.fasta")
export(getSeq(Celegans, elt2_summit_GRange[elementMetadata(elt2_summit_GRange)[,"bound_class"] %in% 
                                      c("ClassA", "Target Gene Class B", "Target Gene Class C")]),
       "./02_output/MEME-ChIP_fasta/BoundDE_NoClassA2_elt2_summits.fasta")
export(getSeq(Celegans, elt2_summit_GRange[elementMetadata(elt2_summit_GRange)[,"bound_class"] %in% 
                                      c("ClassA", "Target Gene Class A2", "Target Gene Class C")]),
       "./02_output/MEME-ChIP_fasta/BoundDE_NoClassB_elt2_summits.fasta")
export(getSeq(Celegans, elt2_summit_GRange[elementMetadata(elt2_summit_GRange)[,"bound_class"] %in% 
                                      c("ClassA", "Target Gene Class A2", "Target Gene Class B")]),
       "./02_output/MEME-ChIP_fasta/BoundDE_NoClassC_elt2_summits.fasta")

```

Use `meme-suite.org` to perform motif analysis.

# Figre SX measure intestine specificity of intestine gene list
```{r}
library(reticulate)
...
intestine_gene_list <- read.csv(file = "../Figure_3_Intestine_Expression_Patterns/02_output/Fig3_Public_Intestine_RNA_Gene_List.csv")
```


```{r fig.width=6, fig.height=3}
intestine_tissue_df <-tea$enrichment_analysis(intestine_gene_list$WBGeneID, tissue_df = tissue_df)
intestine_tissue_df %>% rename(Q.value = `Q value`)  %>%
  filter(Q.value < 0.05,  Observed > 1000) %>% 
  ggplot(aes(x = reorder_within(Term, -log10(Q.value), Term), y = -log10(Q.value))) +
...
# Figure SX: Intestine Specific Classes with L1 binding

```{r}
read_xls("./01_input/Table_S5_Intestine_Specific_Classes.xlsx")
getwd()
intestine_class_genes <- read_xlsx(path = "./01_input/Table_S5_Intestine_Specific_Classes.xlsx")
intestine_class_genes_scaled_matrix <- matrix_select(dynamic_counts_matrix_scaled_ascend, gene_subset_vector = intestine_class_genes$WBGeneID)

intestine_class_genes$ELT2chip_peaksnum_Weisenfahrt <- as.integer(intestine_class_genes$ELT2chip_peaksnum_Weisenfahrt)
intestine_class_genes$ELT2chip_peaksnum_Mann <- as.integer(intestine_class_genes$ELT2chip_peaksnum_Mann)
MannWieseELT2 <- intestine_class_genes %>% 
  mutate(
  ELT2WieseBool = 
    case_when(ELT2chip_peaksnum_Weisenfahrt >= 1 ~ "TRUE", 
              is.na(ELT2chip_peaksnum_Weisenfahrt) ~ "FALSE"),
  ELT2MannBool = 
    case_when(ELT2chip_peaksnum_Mann >=1 ~ "TRUE",
              is.na(ELT2chip_peaksnum_Mann) ~ "FALSE"),
  ELT2_L1 = as.character(WBGeneID %in% l1_bound_list$WBGeneID)
  ) %>% 
  dplyr::select(WBGeneID, gene_id_val, ELT2chip_peaksnum_Weisenfahrt, ELT2WieseBool, ELT2chip_peaksnum_Mann, ELT2MannBool, ELT2_L1)
MannWieseELT2
Heatmap(intestine_class_genes_scaled_matrix,
        col = colorRampPalette(c("cyan", "black", "yellow"))(1000),
        cluster_columns = FALSE,
        show_row_names = FALSE,
        row_split = intestine_class_genes$class) +
  rowAnnotation(WeisenfahrtELT2 = MannWieseELT2$ELT2WieseBool,
                MannELT2 = MannWieseELT2$ELT2MannBool,
                ELT2_L1 = MannWieseELT2$ELT2_L1,
                col = list(WeisenfahrtELT2 = c("TRUE" = "green", "FALSE" = "black"),
                           MannELT2 = c("TRUE" = "green", "FALSE" = "black"),
                           ELT2_L1 = c("TRUE" = "green", "FALSE" = "black")
                           )
                )


rowAnnotation(L1bound = elt2_L1_anno$elt2_detected_in_L1,
                     col = list(L1bound = c(
                       "Direct" = "green", "Indirect" = "black"
                     )))
```
