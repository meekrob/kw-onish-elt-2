...
# Measure enrichment between expression SETs and ELT-2 occupancy clusters
Mosaic plot/dot plot representation of chi-square test

```{r}
tableformosaic <- table(gene_peaks_sets$set, gene_peaks_sets$cluster.description)
dimnames(tableformosaic) <- list("DineenSets" = paste("SET", 1:6, sep = ""), " " =c("Embryo_Specific","Increasing","L3_High","Larval","Not_Changing", "Not_Bound"))
...
q <- p.adjust(
  2*pnorm(
    ...
    lower.tail = FALSE), 
  method = "BH")
tableformosaic.q <- matrix(q, ncol = 6)
dimnames(tableformosaic.q) <- dimnames(tableformosaic)
tableformosaic.q<- as.table(tableformosaic.q)
dineen_ELT2_pearson_residuals <- cbind(dineen_ELT2_pearson_residuals, p.adjust = as.data.frame(tableformosaic.q)$Freq)
...
ggplot(dineen_ELT2_pearson_residuals,
  ...
  theme_bw() +
  theme(legend.key.width = unit(0.125,"in"), legend.key.height = unit(0.11, "in"), legend.key.size = unit(0.01, "in"), axis.text.x = element_text(angle = 315, hjust = -0.01)) +
...
ggsave(paste(plotdir,
          "Dineen_Sets_ELT2clusters_custom_mosaic",
          "_",
          today(),
          ".pdf",
          sep = ""), width = 5, height = 4, useDingbats=FALSE)
}
plot <- TRUE
```
